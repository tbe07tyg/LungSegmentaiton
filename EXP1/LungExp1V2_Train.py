...
def LungGen(inputSeries, labelSeries, input_mean_list_series , input_std_list_series, input_nonzeros_indexs_series, total_samples, batch_size, input_shape, anchors, num_classes, train_flag):
    ...
    while True:
        ...
        if batch_counter == 0 and len(initial_train_targets) == 0:
            initial_train_targets =  ziped_series_mask_list.copy()
            print("full training sereis:", len(initial_train_targets))
            ...
            if len(initial_train_targets)>0:
                if train_flag == "Train": ##  for train
                    ...
                    if len(input_nonzeros_indexs_series) > 0:
                        ...
                        print("the non zero index after removing:", input_nonzeros_indexs_series, len(input_nonzeros_indexs_series))
                    ...
                    selected_zip_series = initial_train_targets[0]  # every time pop from the head
                    inputSeries, labelSeries, input_mean_list_series, input_std_list_series, input_nonzeros_indexs_series = selected_zip_series
                    if len(input_nonzeros_indexs_series) > 0:
                        # 2. read the data and random select one index from nonzero index series
                        selected_slice_index = input_nonzeros_indexs_series[0] # every time pop from the head
                        print("selected nonzero slice index:", selected_slice_index)
                        input_data = extract_series(inputSeries)
                        mask_data  = extract_series(labelSeries)
                        selected_slice_input =  input_data[:,:, selected_slice_index]
...
if __name__ == "__main__":
    ...
    def _main():
        project_name = 'LungExp1_DS{}_{}_{:.2f}'.format(Ds,model_index, ANGLE_STEP)
