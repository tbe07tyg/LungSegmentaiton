...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    # add trainable threshold layer
    # th1_in = tf.identity(inputs)
    # 1 raw input and 2 threshold layer
    #1 threshold

    th1_out = ThresholdLayer()(inputs)
    # print("th1:", th1_out)
    th2_out = ThresholdLayer()(inputs)
    #2 threshold
    # th2 = ThresholdLayer()(inputs)
    # print("th2:", th1)
    # concate
    concate_inputs = Concatenate()([inputs, th1_out, th2_out])
    # inputs = K.concatenate([inputs, inputs, inputs], -1)
    print("inputs of backbone:", inputs)
    # 3 concate input with th1 and th2
    base_model = Xception(input_tensor=concate_inputs, weights=None, include_top=False) # random initialization
...
class YOLO(object):
    ...
    def generate(self):
        ...
        try:
            ...
            self.yolo_model,_ = yolo_body(Input(shape=(512, 512, 1)), anchors_per_level, num_classes)
...
def LungGen(inputSeries, labelSeries, input_mean_list_series , input_std_list_series, input_nonzeros_indexs_series,  batch_size, input_shape, anchors, num_classes, train_flag):
    """
    :param images_list:
    :param masks_list:
    :param batch_size:
    ...
    while True:
        ...
        # step 1： random select one series
        ...
        # Step 2:
        # extract useful informaiton from zip list
        inputSeries, labelSeries, input_mean_list_series, input_std_list_series, input_nonzeros_indexs_series = selected_zip_series
        ...
        while b < batch_size:
            # Step 3. random select one index from nonzero index series
            if train_flag == "Train":
                print("For train:")
            ...
            print("the non zero index after removing:",
                  len(input_nonzeros_indexs_series))
...
if __name__ == "__main__":
    ...
    def _main():
        ...
        bs =4 # batch size
        project_name = 'LungEXP2_faterPipe_DS{}_{}_{:.2f}'.format(Ds,model_index, ANGLE_STEP)
        ...
        print("train warm up ---------------------------->")
        # for each_input_series, each_mask_series in zip(train_input_series, train_mask_series):
        train_mean_list_series, train_std_list_series, train_nonzeros_indexs_series, train_num_batches= \
            warm_up_func(train_input_series, train_mask_series, batch_size=bs)

        val_mean_list_series, val_std_list_series, val_nonzeros_indexs_series, val_num_batches = \
            warm_up_func(val_input_series, val_mask_series, batch_size=bs)
        ...
        model.fit_generator(train_Gen,
                            # steps_per_epoch=max(1, math.ceil(num_train/batch_size)),
                            steps_per_epoch=max(1, train_num_batches),
                            validation_data=val_Gen,
                            validation_steps=max(1, val_num_batches),
                            epochs=epochs,
                            initial_epoch=0,
                            callbacks=[reduce_lr, checkpoint, TensorBoardcallback, deleteOldH5])
    ...
    def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
        ...
        if load_pretrained:
            ...
            # model_body.load_weights(weights_path, by_name=True, skip_mismatch=True) # by_name = true will lead the problem
            pass
        ...
        print("final model input:", model.inputs)
        # print(model.summary())
        return model
