...
try:
    def infer_case(Inference_scripts_root, Saved_model_file_root, output_folder):
        ...
        for each_case in lines:
            if "save" in each_case[0]:
                print("each_case:", each_case)
                avg_fps_element = each_case[-2]

                print("avg_fps_element:", avg_fps_element)
        ...
        for gt_line in gt_lines:
            ...
            if img is None:
                coco_gt['images'].append(
                    {'file_name': gt_img_path, 'id': img_id, 'height': 512, 'width': 512})
            else:
                coco_gt['images'].append(
...
if __name__ == '__main__':
    ...
    # # # AS1.8 backup for angle step initial check
    # # ep042-loss11.430-val_loss3.604
    ...
    # #
    # output_folder = "F:\\AngleStepCheck\\output\\AS1backUp"  # the name path can not be too long
    # infer_case(Inference_scripts_root=Inference_scripts_root,
    #            Saved_model_file_root=Saved_model_file_root,
    #            output_folder=output_folder)
